...
from models.bts.model.models_bts import MVBTSNet, MVBTSNet2, BTSNet, BEVNet  ## default: BTSNet
...
class BTSWrapper(nn.Module):
    ...
    def get_loss_metric_names():
        return [
            ...
            "loss_temporal",
            "loss_pgt",
        ]  ## loss_pgt: pseudo GT, default: ["loss", "loss_l2", "loss_mask", "loss_temporal"]
    ...
    def forward(self, data):
        ...
        if self.enc_style == "random":          ## randomly assign the possible encoder views. This includes fisheyes
            ...
            if self.training:
                size = self.nv_ - 1
                max_val = max(1, v - 5)  # Ensure we have at least 1 element to choose from
                indices = torch.randperm(max_val)[:size] + 1
                encoder_perm = indices.tolist()
                ids_encoder = [0]
                ids_encoder.extend(encoder_perm)
            else:
                ids_encoder = [0]   ## In inference, it's singleview prediction (c.f. buttom left view out of possible 4 viewws at a time)
        elif self.enc_style == "mono":
            ids_encoder = [0]
        else:
        ...
        if self.training:
            if self.frame_sample_mode == "only":
                ...
                if ids_loss == [] or ids_render == []:
                    print("__Warning: ids_loss or ids_render is empty. Automatically assigning [0,1] [2,3].")
                    ids_loss = [0, 2] if not ids_loss else ids_loss
                    ids_render = [1, 3] if not ids_render else ids_render
        ...
        if self.prediction_mode == "multiscale":
            ...
            with profiler.record_function("trainer_render"):
                ...
                ### [n:=batch_size, M_patches, 8]    ## forward in _RenderWrapper
            if "fine" not in render_dict:
                render_dict["fine"] = dict(render_dict["coarse"])
        ...
        if self.training is False:  ## for viz eval in Bird-eye view
...
def initialize(config: dict, logger=None):
    ...
    if arch == "MVBTSNet":
    ...
    elif arch == "BEVNet":  ## Non-official - still testing
        if config["model_conf"]["code_mode"] == "z_feat":   cam_pos = 1
        else:   cam_pos = 0
        code_xyz = PositionalEncoding.from_conf(config["model_conf"]["code"], d_in=3 + cam_pos)
        encoder = make_backbone(config["model_conf"]["encoder"])
        ...
        d_in = (
            BEV_latent_size
            # BEV_latent_size + code_xyz.d_out
        )  ### 103 | 116 (TODO: some issue in ids_encoding embedding in Tensor)
        decoder_heads = {
            head_conf["name"]: make_head(head_conf, d_in, d_out) for head_conf in config["model_conf"]["decoder_heads"]
        }
