...
class PatchRaySampler(RaySampler):
    ...
    def reconstruct(self, render_dict, channels=None):
        ...
        coarse["rgb"] = c_rgb.view(
            n, self._patch_count, self.patch_size_y, self.patch_size_x, v, channels
        )
        coarse["weights"] = c_weights.view(
            n, self._patch_count, self.patch_size_y, self.patch_size_x, c_n_smps
        )
        coarse["depth"] = c_depth.view(
            n, self._patch_count, self.patch_size_y, self.patch_size_x
        )
        coarse["invalid"] = c_invalid.view(
            n, self._patch_count, self.patch_size_y, self.patch_size_x, c_n_smps, v
        )

        fine["rgb"] = f_rgb.view(
            n, self._patch_count, self.patch_size_y, self.patch_size_x, v, channels
        )
        fine["weights"] = f_weights.view(
            n, self._patch_count, self.patch_size_y, self.patch_size_x, f_n_smps
        )
        fine["depth"] = f_depth.view(
            n, self._patch_count, self.patch_size_y, self.patch_size_x
        )
        fine["invalid"] = f_invalid.view(
            n, self._patch_count, self.patch_size_y, self.patch_size_x, f_n_smps, v
        )

        if "alphas" in coarse:
            c_alphas = coarse["alphas"]
            f_alphas = fine["alphas"]
            coarse["alphas"] = c_alphas.view(
                n, self._patch_count, self.patch_size_y, self.patch_size_x, c_n_smps
            )
            fine["alphas"] = f_alphas.view(
                n, self._patch_count, self.patch_size_y, self.patch_size_x, f_n_smps
            )
        ...
        if "z_samps" in coarse:
            c_z_samps = coarse["z_samps"]
            f_z_samps = fine["z_samps"]
            coarse["z_samps"] = c_z_samps.view(
                n, self._patch_count, self.patch_size_y, self.patch_size_x, c_n_smps
            )
            fine["z_samps"] = f_z_samps.view(
                n, self._patch_count, self.patch_size_y, self.patch_size_x, f_n_smps
            )
        ...
        if "rgb_samps" in coarse:
            c_rgb_samps = coarse["rgb_samps"]
            f_rgb_samps = fine["rgb_samps"]
            coarse["rgb_samps"] = c_rgb_samps.view(
                n,
                self._patch_count,
                self.patch_size_y,
                self.patch_size_x,
            ...
            fine["rgb_samps"] = f_rgb_samps.view(
                n,
                self._patch_count,
                self.patch_size_y,
                self.patch_size_x,
...
class ImageRaySampler(RaySampler):
    def __init__(
        self, z_near, z_far, height=None, width=None, channels=3, norm_dir=True
    ):
        self.z_near = z_near
        self.z_far = z_far
        self.height = height
    ...
    def reconstruct(self, render_dict, channels=None):
        ...
        coarse["invalid"] = c_invalid.view(
            n, v_in, self.height, self.width, c_n_smps, v_render
        )
        ...
        fine["invalid"] = f_invalid.view(
            n, v_in, self.height, self.width, f_n_smps, v_render
        )
        ...
        if "z_samps" in coarse:
            ...
            f_z_samps = fine["z_samps"]
            coarse["z_samps"] = c_z_samps.view(
                n, v_in, self.height, self.width, c_n_smps
            )
        ...
        if "rgb_samps" in coarse:
            ...
            f_rgb_samps = fine["rgb_samps"]
            coarse["rgb_samps"] = c_rgb_samps.view(
                n, v_in, self.height, self.width, c_n_smps, v_render, channels
            )
            fine["rgb_samps"] = f_rgb_samps.view(
                n, v_in, self.height, self.width, f_n_smps, v_render, channels
            )
