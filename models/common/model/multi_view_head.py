...
class MultiViewHead3(nn.Module):
    ...
    def forward(
        ...
        if (
            self.dropout.p != 0 and self.do_mvh and invalid_features is not None
        ):  ## dropping out except first view feature map due to pgt_loss computation
            invalid_features = torch.concat(
                [
                    invalid_features[:, :1],
        ...
        elif self.dropout.p != 0 and not self.do_mvh and invalid_features is not None:
            invalid_features = 1 - self.dropout(
                (1 - invalid_features.float())
            )  ## Note: after dropping out NeuRay, the values of elements are 2. ## randomly zero out the valid sampled_features' matrix. i.e. (1-invalid_features)
        elif self.dropout.p == 0 and not self.do_mvh:
            pass
        elif invalid_features is None:
            pass
        else:
            raise NotImplementedError(
                f"__unrecognized self.dropout: {self.dropout}, self.do_mvh: {self.do_mvh} condition"
        ...
        if invalid_features is not None:
            ...
            weights = torch.nn.functional.softmax(encoded_features[..., 0], dim=-1)
...
class DensityHead_BEV(nn.Module):
    def __init__(
        self,
        # mlp: nn.Module,
        mlp2: nn.Module,
        ...
        super(DensityHead_BEV, self).__init__()
    ...
    def forward(
        ...
        encoded_features = self.mlp2(sampled_features)
    ...
    @classmethod
    def from_conf(cls, conf, d_in, d_out):
        # mlp = ResnetFC.from_conf(conf["mlp"]["args"], d_in, conf["mlp2"]["d_in"] + 1, d_out)

        # mlp2 = ResnetFC.from_conf(conf["mlp2"]["args"], conf["mlp2"]["d_in"], d_out)
        mlp2 = ResnetFC.from_conf(conf["mlp2"]["args"], 64, d_out)
