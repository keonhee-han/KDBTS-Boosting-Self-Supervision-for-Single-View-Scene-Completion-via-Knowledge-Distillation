...
class SSIM(nn.Module):
    ...
    def forward(self, x, y, pad=True):
        ...
        mu_x_sq = mu_x ** 2 ## squares of the averages and the product of the averages, respectively.
        mu_y_sq = mu_y ** 2
        mu_x_y = mu_x * mu_y
        ...
        if not self.eval_mode:  ## determines how to handle the output of the SSIM calculation
            if not self.comp_mode:  ## error (1 - SSIM index), used as a loss function during training
                return torch.clamp((1 - SSIM_n / SSIM_d) / 2, 0, 1)
            else:   ## return the SSIM index itself.
                return torch.clamp((1 - SSIM_n / SSIM_d), 0, 1) / 2
        else:
            return SSIM_n / SSIM_d  ## returned error is scaled to range between 0 and 1 for easier interpretation and usage in loss calculations
...
def geo(x, y, pad_reflection=True, gaussian_average=False, comp_mode=False, eval_mode=False, pad=True):
