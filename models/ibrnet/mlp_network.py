...
class IBRNet(nn.Module):
    def __init__(self, args, in_feat_ch=32, n_samples=64, **kwargs):
        ...
        self.base_fc = nn.Sequential(
            nn.Linear((in_feat_ch + 3) * 3, 64), activation_func, nn.Linear(64, 32), activation_func
        )
    ...
    def posenc(self, d_hid, n_samples):
        ...
        sinusoid_table = (
            torch.from_numpy(sinusoid_table).to("cuda:{}".format(self.args.local_rank)).float().unsqueeze(0)
        )
    ...
    def forward(self, rgb_feat, ray_diff, mask):
        ...
        sigma_out = sigma.masked_fill(num_valid_obs < 1, 0.0)  # set the sigma of invalid point to zero
